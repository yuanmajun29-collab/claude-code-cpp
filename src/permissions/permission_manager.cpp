...
bool Sandbox::is_path_allowed(const std::string& path) const {
    ...
    std::string npath = normalized.string();

    // Check blocked paths
    for (const auto& blocked : blocked_paths_) {
        if (util::starts_with(npath, blocked)) return false;
    }

    // If restricted, check allowed paths
    if (level_ == Level::Restricted) {
        if (allowed_paths_.empty()) return true;  // No restrictions
        for (const auto& allowed : allowed_paths_) {
            if (util::starts_with(npath, allowed)) return true;
        }
        return false;
    }
...
bool Sandbox::is_command_allowed(const std::string& command) const {
    ...
    std::string cmd = util::to_lower(command);
    std::string base_cmd = cmd;
    auto space = base_cmd.find(' ');
    if (space != std::string::npos) base_cmd = base_cmd.substr(0, space);

    // Check blocked
    for (const auto& blocked : blocked_commands_) {
        if (base_cmd == util::to_lower(blocked)) return false;
    }
